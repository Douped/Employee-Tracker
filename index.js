...
const prompts = [
    ...
    },
    {
        type: 'input',
        name: 'role_id',
        message: 'Enter the ID of the role to assign the employee to (leave empty if unknown): ',
    },
    {
        type: 'input',
        name: 'manager_id',
        message: 'Enter the ID of the manager to assign to the employee (leave empty if unknown): ',
    },
    {
        type: 'input',
        name: 'employee_id',
        message: 'Enter the ID of the employee whose role you would like to modify: ',
    },
];
...
viewAllEmployees = () => {
    ...
    db.query('SELECT e.id AS ID, e.first_name AS First_Name, e.last_name AS Last_Name, er.title AS Job Title, e.manager_id AS Supervisor_ID FROM employees e JOIN employee_roles er ON e.role_id = er.id',
        (err, result) => {
            if (err) {
                console.log(err);
            } else {
...
addRole = () => {
    inquirer
        .prompt([prompts[2], prompts[3], prompts[4]])
        .then((data) => {
            db.query('INSERT INTO employee_roles (title, salary, department_id) VALUES (?, ?, ?)', [data.role, data.salary, data.department_id], (err, result) => {
                if (err) {
                    console.log(err);
                } else {
...
addEmployee = () => {
    inquirer
        ...
        .then((data) => {
            ...
            db.query('INSERT INTO employees (first_name, last_name, role_id, manager_id) VALUES (?, ?, ?, ?)', [data.first_name, data.last_name, data.role_id, data.manager_id], (err, result) => {
                if (err) {
                    console.log(err);
                } else {
...
updateEmployeeRole = () => {
    inquirer
        .prompt([prompts[9], prompts[7]])
        .then((data) => {
            db.query('UPDATE employees SET role_id = ? WHERE id = ?', [data.role_id, data.employee_id], (err, result) => {
                if (err) {
                    console.log(err);
                } else {
